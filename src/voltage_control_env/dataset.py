...
class SimbenchDataSet(DataSet):
    ...
    def __init__(self, absolute_profiles, clean_threshold=1e-4):
        ...
        From the obtained dataset all datapoints are removed where the maximum active power for all sgens is below the given threshhold (i.e. at night).
        Setting clean_threshold = None disables the cleaning.
        '''
        super().__init__()
        # self.sb_code = sb_code
        # self.net = sb.get_simbench_net(sb_code)
        # self.absolute_profiles = sb.get_absolute_values(self.net, profiles_instead_of_study_cases=True)
    ...
    def _clean_data(self, threshold):
        '''
        Removes all datapoints from the dataset where the max_p_mw is below the given threshold.
        '''
...
if __name__ == '__main__':
    ...
    sb_data = SimbenchDataSet("1-LV-rural1--0-sw")

    # print('length', sb_data.get_length())
    # print('access',sb_data.get_item(14277))
